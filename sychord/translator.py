...
try:  # pragma: no cover - tolerate direct imports
    from . import core
    from .core import (
...
def has_pythonista_ui() -> bool:
    """Expose whether the optional Pythonista UI layer is available."""

    try:
        from . import ui_pythonista  # type: ignore
    except Exception:  # pragma: no cover
        try:
            import ui_pythonista  # type: ignore
        except Exception:
            return False
        else:
            return getattr(ui_pythonista, "HAS_PYTHONISTA_UI", False)
    else:
        return ui_pythonista.HAS_PYTHONISTA_UI


def present_ui() -> None:
    """Launch the Pythonista UI when available."""

    try:
        from . import ui_pythonista  # type: ignore
    except Exception:  # pragma: no cover
        import ui_pythonista  # type: ignore
    if not getattr(ui_pythonista, "HAS_PYTHONISTA_UI", False):
        raise RuntimeError("Interface Pythonista indisponible dans ce contexte.")
    ui_pythonista.present_ui(session_factory=get_session)
